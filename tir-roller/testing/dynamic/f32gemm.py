...
class Module:   
    @T.prim_func
    def main(a: T.handle, B: T.Buffer((1024, 1024), "float32"), c: T.handle):
        T.func_attr({"opt_shapes": {"m": 64}, "tir.is_global_func": T.bool(True), "tir.noalias": T.bool(True)})
        m = T.int32()
        A = T.match_buffer(a, (m, 1024))
        C = T.match_buffer(c, (m, 1024))
        if m <= 32:
            T.call_packed("main_kernel_opt_m_1024", A.data, B.data, C.data, m)
