...
def parse_runtime(log_path, A_layout="col", B_layout="row"):
    ...
    (
        min_fp32_cudacore_runtime,
        min_fp32_tensorcore_runtime,
        min_fp16_cudacore_runtime,
        min_fp16_tensorcore_runtime,
        min_s8_cudacore_runtime,
        min_s8_tensorcore_runtime,
    ) = [
        min(runtime_array) if len(runtime_array) else "not support"
        for runtime_array in (
            fp32_cudacore_runtime,
            fp32_tensorcore_runtime,
            fp16_cudacore_runtime,
            fp16_tensorcore_runtime,
...
def get_and_print(log_path, log_case, A_layout="col", B_layout="row"):
    (
        ...
        min_s8_cudacore_runtime,
        min_s8_tensorcore_runtime,
    ) = parse_runtime(log_path, A_layout, B_layout)
